...
def generate_iam_auth_token(*, host: str, user: str, port: int, region: str) -> str:
    venv_site = BACKEND_DIR / ".venv" / ("Lib" if sys.platform == "win32" else "lib")
    if sys.platform == "win32":
        venv_site = venv_site / "site-packages"
    else:
        import glob

        matches = glob.glob(str(venv_site / "python*/site-packages"))
        venv_site = Path(matches[0]) if matches else venv_site / "site-packages"
...
def _backend_python() -> Path:
    ...
    if candidate.is_file():
        return candidate
    print(
        "Backend venv not found. Activate it first:\n"
